...
async function initDb() {
  ...
  try {
    const interviewSeeds = [
      ...
      },
      {
        ...
        rounds: 4,
        rounds_detail: JSON.stringify([
          { round_name: 'Technical Screen — SQL', description: 'A 90-minute HackerRank test with 2 Hard SQL problems. The first asked for a recursive CTE to find all reporting chains in an org hierarchy. The second was a complex window function problem — NTILE + LAG on credit score movement over time. Higher bar than any other company I interviewed at.' },
          { round_name: 'dbt & Analytics Engineering Round', description: 'They shared a messy raw data model and asked me to design the staging, intermediate, and mart layers in dbt. I had to explain how I would write dbt tests, document models, and handle slowly changing dimensions. Knowing dbt deeply was non-negotiable here.' },
          { round_name: 'Product Analytics Round', description: 'Case study: "CRED Coin redemption rate dropped 18% last month — diagnose." I walked through a structured approach: define the metric tree, identify leading indicators, segment by coin type and user segment, and build a before/after query to measure the drop. The interviewer pushed back on every assumption which was intense.' },
          { round_name: 'Final — Director of Data', description: 'A strategic conversation about my career goals, how I approach building data culture in product teams, and a past project where data directly influenced a business decision. They want people with strong opinions and business intuition, not just technical skills.' },
        ]),
        overall_experience: 'CRED has the highest SQL bar I have seen in any company. The dbt round completely caught me off-guard — I had done some dbt but not deeply enough. I had to be honest about my gaps and explain how I would learn on the job. Surprisingly, they appreciated the honesty. The culture values intellectual curiosity and they want people who can think independently. The compensation was the best offer I received, but the work demands are equally high. Would recommend preparing dbt and recursive SQL much more than I did.',
        tips: 'Practice recursive CTEs until they feel natural — CRED uses them heavily for their graph-structured data (credit scores, rewards networks). Do at least one real dbt project before interviewing. For product rounds, use the metric tree framework: north star → leading indicators → root causes.',
        interview_date: '2024-03',
      },
      {
        id: 'ie-010',
        ...
        rounds: 3,
        rounds_detail: JSON.stringify([
          { round_name: 'SQL Online Test', description: '45 minutes, 4 SQL questions on HackerRank. Q1: find customers who ordered from 3+ different cuisines. Q2: calculate restaurant repeat order rate. Q3: 7-day rolling average of orders per city. Q4: identify restaurants with delivery time > 20% above the city median. All required window functions or aggregations — nothing basic.' },
          { round_name: 'Take-Home Assignment', description: 'Swiggy gave me an anonymised dataset of 50,000 restaurant orders over 3 months and asked me to: (1) identify top 5 underperforming cities by repeat order rate, (2) find the optimal delivery slot for demand forecasting, (3) build a churn prediction feature list. I had 3 days. I used Python (Pandas + Seaborn) and submitted a Jupyter notebook with a brief slide deck summary.' },
          { round_name: 'Assignment Review + Culture Round', description: 'Two interviewers — one data, one product. The data interviewer went deep on my methodology: "Why did you choose this metric for churn?" and "What would change if we had real-time data?" The product interviewer asked how I would present these findings to a non-technical audience. Culture questions were around handling ambiguity and working in fast-paced environments.' },
        ]),
        overall_experience: 'Swiggy interview process is refreshingly practical — they want to see how you actually work with data, not just if you can pass a SQL test. The take-home assignment was the core of the entire process. I spent about 12 hours on it and put real effort into the presentation layer — not just analysis but insights and recommendations. That made the difference. The team is young, moves fast, and genuinely cares about data quality. Great company to start a data career at.',
        tips: 'For the take-home: do not just run queries and paste outputs. Tell a story with your findings. Use a dashboard or slide deck to present insights clearly. The product round tests whether you can communicate data to non-technical stakeholders — practice translating SQL outputs into business recommendations.',
        interview_date: '2024-01',
      },
      {
        id: 'ie-011',
        ...
        rounds: 5,
        rounds_detail: JSON.stringify([
          { round_name: 'Recruiter Call', description: 'Standard 20-minute screen. Recruiter explained the 5-round loop process and that one round would be with a Bar Raiser from a different team. Asked about my experience with SQL, data modeling, and Python.' },
          { round_name: 'Online Assessment', description: 'Two coding problems: (1) SQL to find the 3rd highest salary per department using dense_rank — solved it. (2) Python/Pandas problem asking me to calculate rolling 7-day retention rate — I missed an edge case and did not pass this cleanly.' },
          { round_name: 'Technical SQL + Data Modeling', description: 'Asked to design a data model for Amazon Prime Video watch history. Then wrote SQL for finding users who started a show but did not complete it within 7 days. Also a question on handling NULL values in OUTER JOINs — when and why NULLs appear. Felt good about this round.' },
          { round_name: 'Bar Raiser Round — Leadership Principles', description: 'This was the hardest round. The Bar Raiser asked deeply probing follow-up questions on every STAR answer. "What would you do differently?" "How do you know that decision was yours and not your manager\'s?" "Give me the actual number, not an approximation." I felt unprepared for the level of scrutiny.' },
          { round_name: 'Final — Hiring Manager', description: 'Discussed past projects and data infrastructure I had built. The manager was impressed technically but mentioned they were looking for someone with more ownership stories. This aligned with the rejection feedback I received later.' },
        ]),
        overall_experience: 'I came close — the rejection was specifically around Leadership Principles depth, not technical skills. Amazon evaluates LPs with a completely different intensity than other companies. Looking back, I had technically strong stories but lacked the quantified business impact that Amazon expects. I reapplied 8 months later with much stronger LP stories and cleared. The lesson: prepare your behavioral stories with the same rigor as your SQL — both are make-or-break at Amazon.',
        tips: 'Do not underestimate Amazon Leadership Principles. Prepare 2-3 STAR stories per principle, each with specific metrics (e.g. "reduced report generation time by 60%, saving the team 8 hours per week"). The Bar Raiser will probe until you have no more detail — prepare to go 4-5 levels deep on every story.',
        interview_date: '2023-09',
      },
    ];
  ...
  try { await c.execute('CREATE TABLE IF NOT EXISTS placement_companies (id TEXT PRIMARY KEY, name TEXT NOT NULL, logo TEXT DEFAULT \'🏢\', color TEXT DEFAULT \'#4A90D9\', industry TEXT DEFAULT \'Technology\', roles TEXT DEFAULT \'[]\', difficulty TEXT DEFAULT \'Hard\', interview_rounds TEXT DEFAULT \'[]\', key_topics TEXT DEFAULT \'[]\', prep_tips TEXT DEFAULT \'[]\', salary_range TEXT DEFAULT NULL, success_rate INTEGER DEFAULT 20, is_active INTEGER DEFAULT 1, created_at TEXT DEFAULT (datetime(\'now\')))'); } catch(e) {}
