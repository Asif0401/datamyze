...
router.post('/force-seed', authMiddleware, adminOnly, async (req, res) => {
  ...
  try {
    // Ensure tables exist
    await run(db, `CREATE TABLE IF NOT EXISTS company_banks (id TEXT PRIMARY KEY, name TEXT NOT NULL UNIQUE, logo TEXT DEFAULT '🏢', color TEXT DEFAULT '#4A90D9', industry TEXT DEFAULT 'Technology', question_count INTEGER DEFAULT 0, difficulty TEXT DEFAULT 'Medium', is_active INTEGER DEFAULT 1, created_at TEXT DEFAULT (datetime('now')))`);
    await run(db, `CREATE TABLE IF NOT EXISTS company_bank_questions (id TEXT PRIMARY KEY, company_id TEXT NOT NULL, title TEXT NOT NULL, question TEXT NOT NULL, type TEXT DEFAULT 'SQL', difficulty TEXT DEFAULT 'Medium', topic TEXT DEFAULT 'Joins', approach TEXT, xp_reward INTEGER DEFAULT 30, order_index INTEGER DEFAULT 0, created_at TEXT DEFAULT (datetime('now')), UNIQUE(company_id, title))`);
    ...
    for (const co of companies) {
      try {
        ...
        log.push(`company: ${co.name}`);
      } catch(e) { log.push(`FAIL company ${co.name}: ${e.message}`); }
    }

    // Reload companies to get real IDs (INSERT OR IGNORE may have skipped if already exists)
    ...
    const qs = [
      ...
      ['Amazon India','Seller Health Score','Design a Seller Health Score combining delivery %, return rate, rating, and response time.','Analytical','Hard','Metric Design','Normalize each 0-1. Weights: delivery 30%, returns 25%, rating 30%, response 15%. Flag score < 0.6.',40,6],
      ['Amazon India','Cross-sell Opportunities','Find top 10 product pairs frequently bought together with their co-occurrence count and lift.','SQL','Hard','Market Basket','Self-join order_items on order_id. Lift = P(A and B)/(P(A)*P(B)).',40,7],
      // Swiggy
      ...
      ['Swiggy','Python: Surge Pricing Model','Write Python to calculate a surge multiplier (1x to 3x) from historical order volume and driver availability.','Python','Hard','Business Logic','surge = demand/supply ratio. ratio > 2.0 -> 3x, > 1.5 -> 2x, > 1.2 -> 1.5x, else 1x. Smooth with rolling average.',40,5],
      ...
      ['Zomato','RFM User Segmentation','Segment users into High/Medium/Low LTV using RFM model.','SQL','Hard','RFM Analysis','Recency = DATEDIFF(today, MAX(order_date)). Frequency = COUNT(orders). Monetary = SUM(value). NTILE(3) per metric. Sum = RFM segment.',40,6],
      // PhonePe
      ...
      ['PhonePe','Fraud Detection Signals','Flag potentially fraudulent accounts: >10 failed txns in 1 hour, >5 device_ids in a day, or state mismatch.','SQL','Hard','Window Functions','COUNT(*) OVER (PARTITION BY user_id ORDER BY txn_time RANGE INTERVAL 1 HOUR). Flag users exceeding thresholds.',40,2],
      ...
      ['Razorpay','Python: Revenue Forecasting','Forecast next 3 months of payment volume using Python.','Python','Hard','Forecasting','Use Facebook Prophet or ARIMA. prophet.fit(df). prophet.predict(future). Evaluate with MAPE. Handle festival spikes.',40,5],
      ...
      ['Meesho','Viral Coefficient','Calculate viral coefficient K = (invites sent per user) x (conversion rate of invites). If K > 1, growth is viral.','SQL','Medium','Business Metric','AVG(invites_sent) x (COUNT(successful_referrals) / COUNT(invites_sent)). Single query with two aggregations.',30,2],
      ['Meesho','Return Abuse Detection','Find users with return rate > 60% over last 90 days with at least 10 orders.','SQL','Easy','Aggregations','GROUP BY user_id. HAVING returns/orders > 0.6 AND COUNT(orders) >= 10. WHERE order_date >= TODAY - 90.',20,3],
      ...
      ['Walmart Global Tech','Demand Forecast Accuracy','Compare forecasted vs actual demand by SKU. Calculate MAPE per category.','SQL','Hard','Statistics','SUM(ABS(actual-forecast)/actual)*100/COUNT(*) = MAPE per category. JOIN forecast and actuals on sku + week.',35,2],
      ...
      ['CRED','Bill Payment Prediction','Which members are likely to miss their credit card bill payment this month?','Analytical','Hard','ML Design','Features: payment_history, outstanding/limit, income_proxy, utilization. Logistic regression or XGBoost. Use precision-recall (class imbalance).',40,3],
      ...
      ['CRED','Python: Reward Recommendation','Recommend top 3 rewards for a new member using collaborative filtering.','Python','Hard','ML','User-item matrix of redemptions. cosine_similarity or SVD. k nearest neighbors. Use surprise or sklearn.',40,5],
      // Dream11
      ...
      ['Dream11','Team Duplication Rate','What % of teams for a match have identical 11-player selection?','SQL','Hard','String Matching','Hash sorted player list into canonical string per team. COUNT(DISTINCT canonical)/COUNT(total). 1 - this = duplication.',40,2],
      ...
      ['Dream11','Python: Optimal Team Builder','Select optimal 11 players within a Rs 100 budget to maximize predicted_points.','Python','Hard','Optimization','PuLP linear programming or greedy. Constraints: budget, position requirements. Maximize SUM(predicted_points).',40,5],
    ];

    let inserted = 0;
    for (const [coName, title, question, type, diff, topic, approach, xp, order] of qs) {
      const coId = cmap[coName];
      if (!coId) { log.push(`SKIP (no company): ${coName} - ${title}`); continue; }
      try {
        await run(db,
        ...
        inserted++;
      } catch(e) { log.push(`FAIL q "${title}": ${e.message}`); }
    }
    ...
    const finalCos = await all(db, 'SELECT * FROM company_banks');
    res.json({ success: true, companies: finalCos.length, questions_inserted: inserted, log });
  } catch (e) {
