    ...
    if (line.trim().startsWith('```')) {
      ...
      els.push(
        <div key={`cb${i}`} style={{ marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(0,0,0,0.4)', borderRadius: '6px 6px 0 0', padding: '4px 12px', borderBottom: '1px solid rgba(255,255,255,0.12)' }}>
            <span style={{ fontSize: 10, color: '#888', fontFamily: 'monospace', textTransform: 'uppercase', letterSpacing: '.05em' }}>{lang}</span>
          </div>
          <pre style={{ background: 'rgba(0,0,0,0.45)', border: '1px solid rgba(255,255,255,0.12)', borderTop: 'none', borderRadius: '0 0 6px 6px', padding: '12px 16px', overflowX: 'auto', fontSize: 12.5, fontFamily: '"Fira Code",monospace', lineHeight: 1.65, color: '#cdd6f4', margin: 0 }}>
            <code>{code.join('\n')}</code>
          </pre>
        </div>
      );
...
function LiveSessionPanel({ courseId, courseName, initialVoted = false }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
      ...
      {/* What you get */}
      <div style={{ background: 'rgba(255,255,255,0.09)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 14, padding: '1.2rem 1.4rem' }}>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        {[
          ...
          <div key={s.step} style={{ flex: 1, background: 'rgba(255,255,255,0.09)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 12, padding: '10px 12px', textAlign: 'center' }}>
      ...
      <div className="modal" style={{ width: 680, maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
        ...
        {activeLesson ? (
          <div style={{ flex: 1, overflowY: 'auto', paddingRight: 4 }}>
            <button onClick={() => setActiveLesson(null)} style={{ background: 'rgba(255,255,255,0.09)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: '5px 14px', fontSize: 12, color: 'var(--muted)', cursor: 'pointer', marginBottom: '1rem' }}>
              ← Back to course
            </button>
            ...
            {/* Content */}
            <div style={{ background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: '1.2rem 1.4rem', marginBottom: '1.2rem' }}>
          ...
          <>
            ...
            {/* Tabs */}
            <div style={{ display: 'flex', gap: 4, borderBottom: '1px solid rgba(255,255,255,0.12)', marginBottom: '1.2rem' }}>
              {TABS.map(t => (
            ...
            <div style={{ flex: 1, overflowY: 'auto', paddingRight: 4 }}>
              ...
              {tab === 'overview' && (
                <div>
                  ...
                  {/* Prerequisites */}
                  <div style={{ marginBottom: '1.2rem', background: 'rgba(255,255,255,0.09)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, padding: '10px 14px' }}>
                  ...
                  <div style={{ marginBottom: '1.2rem' }}>
                    ...
                    <div style={{ display: 'grid', gap: 4 }}>
                      {lessons.map((l, idx) => (
                        <div key={l.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 10px', background: 'rgba(255,255,255,0.09)', borderRadius: 7, fontSize: 12.5 }}>
            ...
            {/* CTA */}
            <div style={{ marginTop: '1.2rem', paddingTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.12)' }}>
              {!progress ? (
